...
一开始遇到了一个问题，编译时一直失败，一直失败，和DS研究半天发现是VScode被snap污染了（因为VScode是从snap上下载的），导致文件路径中总混入了个snap，于是我们研究怎么把snap从路径里清除掉，但是snap十分顽固，根本清理不掉，最后DS给了两个最后的解决方案，写一个脚本或者删除VScode再从官网上下载，这样就不会被污染了，我先尝试了脚本，虽然每次编译都有点小烦，但也是成功看到图像了，本以为问题已解决，但当我运行视频的代码时输出结果还是这个图像我就意识到不对劲了，DS又告诉我，一个脚本只能用于一个文件，那就不是一点烦了，是亿点烦啊，于是我果断删除VScode然后重新下载了，终于解决了问题。整个过程耗时2个晚上。
